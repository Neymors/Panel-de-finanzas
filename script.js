 ...
 * Vanilla JS | Local-First | BYMA + Yahoo + CoinGecko
 */
'use strict';
...
function saveDailySnapshot(totalUSD) {
    ...
    history.push({
        ...
        benchmark: Math.round(totalUSD * (1 + benchmarkChange / 100))
    });
...
function renderLineChart(processed) {
    ...
    const history = (Storage.get(CONFIG.LS_HISTORY) || []).filter(h => {
        const date = new Date(h.date);
        const now = new Date();
        const diff = (now - date) / (1000 * 60 * 60 * 24);
        if (State.activeRange === '1m') return diff <= 30;
        if (State.activeRange === '6m') return diff <= 180;
        if (State.activeRange === '1y') return diff <= 365;
        if (State.activeRange === 'ytd') return date.getFullYear() === now.getFullYear();
        return true;
    });

    const labels = history.length > 0 
        ? history.map(h => Format.date(h.date))
...
function injectControls() {
    ...
    controls.innerHTML = `
        <button class="ctrl-btn" id="btnExport" title="Exportar portfolio">📦 Export</button>
        <button class="ctrl-btn" id="btnImport" title="Importar portfolio">📥 Import</button>
    `;
